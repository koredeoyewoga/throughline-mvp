...
| Ownership ambiguity is surfaced, never guessed | a `Handoff` does not change `Task.assignee` until the receiving owner explicitly acknowledges it (`engine/handoffs.ts`); an unacknowledged handoff is reported as "pending acknowledgement" and, past a configurable window, "Owner unknown" — a visible coordination risk rather than a silent assumption |
| Blockers are person-reported, not AI-inferred | `engine/blockers.ts` — a `Blocker` only exists because someone named it; it is never created by the detection engine, so it cannot be used to fabricate evidence for a recommendation |
